...
class GameRating:
    ...
    @staticmethod
    def pre_process(data, to_standardize=False, use_pca=False):
    ...
    def train(self):
        ...
        # pre-process data
        modify = self.pre_process(data, to_standardize=True, use_pca=True)
        ground_truth = data.get('playtime_forever')

        # slice data
        x_train, x_val, y_train, y_val = train_test_split(modify, ground_truth, test_size=0.167,
                                                          random_state=int(time.time()))

        # convert training data into LightGBM data set format
        d = lgb.Dataset(modify, label=ground_truth)
        ...
        # lgb_model = lgb.train(params, d_train, num_boost_round=24, valid_sets=d_val)
        lgb_model = lgb.train(params, d, num_boost_round=24, valid_sets=d_val)
    ...
    def validate(self):
        ...
        # convert training data into LightGBM data set format
        d_train = lgb.Dataset(modify, label=ground_truth)
    ...
    def predict():
        ...
        # perform standardization
        modify = GameRating.pre_process(data, True, True)
        ...
        playtime = clf.predict(x_test)

        data['playtime_forever'] = playtime

        data = data[['id', 'playtime_forever']]

        data['playtime_forever'] = data['playtime_forever'].apply(lambda x: max(0, x))
